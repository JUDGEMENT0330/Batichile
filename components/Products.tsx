          ...
          {PRODUCTS.map((product, index) => (
            ...
            >
              ...
              <div className={`relative w-full aspect-square border-4 border-black shadow-comic ${product.color} overflow-hidden mb-4 transition-transform hover:-translate-y-2 duration-300`}>
                ...
                <div className="absolute top-2 right-2 bg-black text-white text-xs font-bold px-2 py-1 transform rotate-2 z-20 border border-white shadow-sm">
                   NIVEL: {product.level}
                </div>

                {/* Character Image Container - Full Focus */}
                <div className="absolute inset-0 flex items-end justify-center">
                    {/* Utilizamos solo characterImage, optimizado para llenar la viñeta */}
                    <img 
                        src={product.characterImage} 
                        alt={product.characterName}
                        className="h-[95%] w-auto max-w-full object-contain drop-shadow-[5px_5px_0px_rgba(0,0,0,0.3)] group-hover:scale-110 transition-transform duration-300 filter contrast-125"
                    />
                </div>

                {/* Comic Action Text/Onomatopoeia */}
                <div className="absolute bottom-3 left-3 bg-white border-2 border-black px-2 py-0.5 text-xs font-black transform -rotate-6 z-20 shadow-[3px_3px_0_#000]">
              ...
              <div className="bg-white border-4 border-black p-4 flex-grow flex flex-col relative">
                 ...
                 <h3 
                   className={`text-4xl comic-font mb-1 ${product.titleColor} text-stroke-black -mt-8 bg-white inline-block self-start px-3 py-1 border-2 border-black transform -rotate-2 shadow-[3px_3px_0_#000] z-30`}
                 >
                   {product.name}
                 </h3>
